...
s3 = boto3.client('s3') # uing execution role to access s3
...
# whenever an event change happens in S3 it will invoke this function and pass events details 
def lambda_handler(event, context):
	# Get bucket name from the event - if it matches a particular bucket name ensure it is private and prevent users making access public
